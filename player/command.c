...
static int get_track_entry(int item, int action, void *arg, void *ctx)
{
    ...
    struct m_sub_property props[] = {
        ...
        {"lang2",       SUB_PROP_STR(track->stream->lang_sub),
                        .unavailable = !(track->stream &&
                                         track->stream->is_dmono &&
                                         track->stream->lang_sub),},
        {"dual-mono",   SUB_PROP_INT(track->stream->dmono_mode),
                        .unavailable = !(track->stream &&
                                         track->stream->is_dmono),},
                               ...
                               int action, void *arg)
{
    ...
    case M_PROPERTY_SET:
        ...
        struct track *t;
        t = mp_track_by_dmxid(mpctx, STREAM_AUDIO, prog.dmxid[STREAM_AUDIO]);
...
static void update_track_switch(struct MPContext *mpctx, int order, int type)
{
    ...
    if (tid == -1) {
        ...
        track = select_default_track(mpctx, order, type, -1);
