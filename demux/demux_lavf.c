...
static void check_pid_change(demuxer_t *demuxer)
{
    ...
    // detected as a lost pid already... Bypass pid check
...
static void update_metadata(demuxer_t *demuxer)
{
    ...
    if (priv->new_prog_set) {
        int p;

        for (p = 0; p < priv->avfc->nb_programs; p++)
            if (priv->avfc->programs[p]->id == priv->cur_program)
                break;
        if (p < priv->avfc->nb_programs)
            mp_tags_copy_from_av_dictionary(demuxer->metadata,
                                            priv->avfc->programs[p]->metadata);
        priv->new_prog_set = false;
        updated |= true;
    }
    ...
    for (int n = 0; n < priv->num_streams; n++) {
        ...
        if (st && st->event_flags & AVSTREAM_EVENT_FLAG_METADATA_UPDATED) {
            struct sh_stream *sh = priv->streams[n]->sh;

            if (!sh)
                continue;
            st->event_flags = 0;
            mp_tags_clear(sh->tags);
            mp_tags_copy_from_av_dictionary(sh->tags, st->metadata);
            update_stream_props(sh, sh->tags);
        }
    }
    if (updated)
...
{
    for (int i = 0; i < priv->num_streams; i++)
        if (priv->streams && priv->streams[i] && priv->streams[i]->sh &&
            priv->streams[i]->sh->demuxer_id == id)
            return priv->streams[i]->sh;
...
static void set_current_program(struct demuxer *demuxer)
{
    ...
    prog = NULL;
    for (int n = 0; n < priv->num_streams; n++) {
        struct stream_info *si = priv->streams[n];
...
static bool
demux_lavf_find_next_track(demuxer_t *demuxer, demux_next_track_t *arg)
{
    lavf_priv_t *priv = demuxer->priv;
    ...
    if (!prog) {
        ...
        for (int i = 0; i < priv->num_streams; i++) {
            ...
            if (j != i || arg->wrap)
                arg->ret_id = priv->streams[j]->sh->demuxer_id;
            return true;
        }
        return false;
    }
...
// if s1 is better than s2 (as a default stream)
static bool
is_better_sh(struct sh_stream *s1, struct sh_stream *s2, int pid, char **langs)
{
    if (pid >= 0)
        return s1->demuxer_id == pid;
...
static bool demux_lavf_identify_program(struct demuxer *demuxer, demux_program_t *prog)
{
    lavf_priv_t *priv = demuxer->priv;
...
redo:
    ...
    for (i = 0; i < program->nb_stream_indexes; i++) {
        ...
        if (stream && stream->sh) {
            ...
            case STREAM_VIDEO:
                if (is_better_sh(stream->sh, v_sh,
                        prog->dmxid[STREAM_VIDEO], prog->langs[STREAM_VIDEO]))
                    v_sh = stream->sh;
                break;
            case STREAM_AUDIO:
                if (is_better_sh(stream->sh, a_sh,
                        prog->dmxid[STREAM_AUDIO], prog->langs[STREAM_AUDIO]))
                    a_sh = stream->sh;
                break;
            case STREAM_SUB:
                if (is_better_sh(stream->sh, s_sh,
                        prog->dmxid[STREAM_SUB], prog->langs[STREAM_SUB]))
                    s_sh = stream->sh;
                break;
            }
        }
    ...
    if (prog->progid <= -1 &&
        ((prog->dmxid[STREAM_VIDEO] >= 0 && !v_sh) ||
         (prog->dmxid[STREAM_AUDIO] >= 0 && !a_sh) ||
         (!v_sh && !a_sh))) {
        p = (p + 1) % priv->avfc->nb_programs;
        if (p == start)
            return false;
    ...
    prog->dmxid[STREAM_VIDEO] = -2;
    prog->dmxid[STREAM_AUDIO] = -2;
    prog->dmxid[STREAM_SUB] = -2;
    if (v_sh)
...
const demuxer_desc_t demuxer_desc_lavf = {
    ...
    .identify_program = demux_lavf_identify_program,
    .find_next_track = demux_lavf_find_next_track,
};
