...
(*) 字幕表示機能は本家側がffmpeg/libaribcaptionに対応したので、`isdb-0.38`以降そちらを利用することとし、
  従来からの(ASS拡張による)独自実装は削除した。

なお本修正はLinuxにのみ対応する。
### 本ブランチ`isdb-0.40`での主な変更点
* 本家mpvの`master`ブランチのv0.40.0タグにリベース
...
```
(*) 最後のリンク段階で、`GL*`関係のシンボルで`undefined symbol`のエラーが出る場合は、以下で解決できる可能性がある。
```
# sed '/^Libs.private/ s|$| /usr/lib/libSPRIV-Tools.so /usr/lib/libSPRIV-Tools-opt.so|' /usr/lib/pkgconfig/libplacebo.pc  \
