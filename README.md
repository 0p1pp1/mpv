...
なお本修正はLinuxにのみ対応する。
### 本ブランチ`isdb-0.34`での主な変更点
* 本家mpvの`master`ブランチ 2022/01/23時点 (v0.34後)にリベース
* 依存ライブラリffmpeg,libassの利用方法を変更し、システムにインストール済みのものを利用する
* 上記変更に伴い、今までのビルドスクリプト`isdb-build`が不要になった
* (依存ライブラリのISDB向けパッチも最新版にリベース. ffmpeg:5.0, libass:0.15.2)
...
```
cd <somewhere>
git clone [--depth 1] -b isdb-0.15.2 https://github.com/0p1pp1/libass
...
```
cd <somewhere>
git clone [--depth 1] -b isdb-5.0 https://github.com/0p1pp1/ffmpeg
cd ffmpeg
mkdir build; cd build
...
```
cd <somewhere>
git clone [--depth 1] -b isdb-0.34 https://github.com/0p1pp1/mpv
cd mpv
